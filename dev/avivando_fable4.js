/* ============================================================================
   AVIVANDO EL FUEGO — FABLE v12 "FUEGO DE LA MAÑANA" (claro · iglesia)
   Misma estructura moderna del v11 (tipo monumental + cursor que aviva el
   fuego + ticker editorial), pero en LUZ: fondo marfil de amanecer y las
   letras en tinta café que ARDEN donde pasa la mano (tinta → brasa → oro).
     · Un shader crudo y liviano (sin Three.js).
     · Audio intocable · Media Session · pausa en oculto · FPS limitado.
   ========================================================================== */
(function () {
  'use strict';
  if (window.__afThemeV12) return;
  window.__afThemeV12 = true;
  ...
  var Blaze = (function () {
    ...
    var FRAG = [
      ...
      ' float smolder=m*(0.10+0.09*n2+0.04*sin(t*1.3+uv.x*9.0));',
      ...
      // calor total (sin suelo oscuro: en claro el suelo es un rubor dorado)
      ' float heat=clamp(smolder+ign+fl+cloud,0.0,1.5);',
      // paleta para fondo CLARO: fuego saturado (carmesí → naranja → oro)
      ' vec3 fireCol=vec3(0.55,0.10,0.02);',
      ' fireCol=mix(fireCol,vec3(0.90,0.26,0.02),smoothstep(0.14,0.44,heat));',
      ' fireCol=mix(fireCol,vec3(1.0,0.55,0.06),smoothstep(0.42,0.78,heat));',
      ' fireCol=mix(fireCol,vec3(1.0,0.78,0.22),smoothstep(0.76,1.10,heat));',
      ' fireCol=mix(fireCol,vec3(1.0,0.92,0.55),smoothstep(1.10,1.4,heat));',
      // dentro de las letras: TINTA café que se vuelve fuego con el calor
      ' vec3 inkCol=mix(vec3(0.16,0.09,0.06), fireCol, smoothstep(0.14,0.55,heat));',
      ' vec3 col=mix(fireCol, inkCol, m);',
      // rubor dorado del amanecer en la base
      ' float dawn=exp(-uv.y*7.0)*0.16;',
      // chispas doradas + grano de papel
      ' float sp=step(0.9958,hash(floor(vec2(uv.x*130.0,(uv.y - t*0.5)*80.0))))*smoothstep(0.15,0.6,heat);',
      ' col=mix(col,vec3(0.85,0.55,0.10),sp*0.9);',
      ' float flameOut=(1.0-m)*clamp((fl+cloud)*1.6-0.14,0.0,1.0);',
      ' float alpha=clamp(m*0.97 + flameOut + sp*0.8 + dawn*(1.0-m)*0.5, 0.0, 1.0);',
      ' float grain=(hash(uv*vec2(917.0,533.0)+fract(t)*7.0)-0.5)*0.05;',
      ' col+=grain*alpha;',
